...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/notificationsnss/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show NotificationSns {item && item["@id"]}</h1>
      ...
      <Link to="/notificationsnss/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/notificationsnss/edit/${encodeURIComponent(item["@id"])}`}>
