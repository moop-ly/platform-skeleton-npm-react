...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/audiotypes/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show AudioType {item && item["@id"]}</h1>
      ...
      <Link to="/audiotypes/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/audiotypes/edit/${encodeURIComponent(item["@id"])}`}>
