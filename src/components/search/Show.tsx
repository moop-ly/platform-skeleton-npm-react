...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/searchs/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show Search {item && item["@id"]}</h1>
        ...
        <table className="table table-responsive table-striped table-hover">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{item["indexes"]}retrieved</td>
            </tr>
            <tr>
              <th scope="row">query</th>
              <td>{item["query"]}retrieved</td>
            </tr>
          </tbody>
        </table>
      )}
      <Link to="/searchs/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/searchs/edit/${encodeURIComponent(item["@id"])}`}>
