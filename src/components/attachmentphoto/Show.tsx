...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/attachmentphotos/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show AttachmentPhoto {item && item["@id"]}</h1>
      ...
      <Link to="/attachmentphotos/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/attachmentphotos/edit/${encodeURIComponent(item["@id"])}`}>
