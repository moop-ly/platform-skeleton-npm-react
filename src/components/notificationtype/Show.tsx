...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/notificationtypes/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show NotificationType {item && item["@id"]}</h1>
      ...
      <Link to="/notificationtypes/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/notificationtypes/edit/${encodeURIComponent(item["@id"])}`}>
