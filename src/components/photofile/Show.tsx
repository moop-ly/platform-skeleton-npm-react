...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/photofiles/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show PhotoFile {item && item["@id"]}</h1>
      ...
      <Link to="/photofiles/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/photofiles/edit/${encodeURIComponent(item["@id"])}`}>
