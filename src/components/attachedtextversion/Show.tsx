...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/attachedtextversions/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show AttachedTextVersion {item && item["@id"]}</h1>
      ...
      <Link to="/attachedtextversions/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link
          to={`/attachedtextversions/edit/${encodeURIComponent(item["@id"])}`}
        >
