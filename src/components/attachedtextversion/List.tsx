...
const ListView = ({ error, loading, retrieved }: ListProps) => {
  ...
  return (
    <div>
      <h1>AttachedTextVersion List</h1>
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item["@id"]}>
              ...
              <td>
                <Link
                  to={`/attachedtextversions/show/${encodeURIComponent(
                    item["@id"]
                  )}`}
                >
              ...
              <td>
                <Link
                  to={`/attachedtextversions/edit/${encodeURIComponent(
                    item["@id"]
                  )}`}
                >
...
const List = () => {
  ...
  const id = (page && decodeURIComponent(page)) || "attached-text-versions";
