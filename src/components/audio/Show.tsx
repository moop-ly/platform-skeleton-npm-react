...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/audios/" replace />;
  }
  ...
  return (
    <div>
              ...
              <td>
                <Links
                  items={item["attachments"].map((ref: any) => ({
                    href: `/attachment-audios/show/${encodeURIComponent(ref)}`,
                    name: ref,
                  }))}
                />
              </td>
      ...
      <Link to="/audios/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/audios/edit/${encodeURIComponent(item["@id"])}`}>
