...
const CreateView = ({
  ...
  if (created) {
    return (
      <Navigate
        to={`/externalcontents/edit/${encodeURIComponent(created["@id"])}`}
        replace
      />
    );
  }
  ...
  return (
    <div>
      ...
      <Link to="/externalcontents/" className="btn btn-primary">
        Back to list
      </Link>
    </div>
  );
