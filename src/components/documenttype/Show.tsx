...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/documenttypes/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show DocumentType {item && item["@id"]}</h1>
      ...
      <Link to="/documenttypes/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/documenttypes/edit/${encodeURIComponent(item["@id"])}`}>
