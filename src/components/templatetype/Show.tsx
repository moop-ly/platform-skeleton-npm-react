...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/templatetypes/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show TemplateType {item && item["@id"]}</h1>
              ...
              <td>
                <Links
                  items={item["templates"].map((ref: any) => ({
                    href: `/templates/show/${encodeURIComponent(ref)}`,
                    name: ref,
                  }))}
                />
              </td>
      ...
      <Link to="/templatetypes/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/templatetypes/edit/${encodeURIComponent(item["@id"])}`}>
