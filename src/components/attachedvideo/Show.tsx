...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/attachedvideos/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show AttachedVideo {item && item["@id"]}</h1>
      ...
      <Link to="/attachedvideos/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/attachedvideos/edit/${encodeURIComponent(item["@id"])}`}>
