...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/notificationsmtps/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show NotificationSmtp {item && item["@id"]}</h1>
      ...
      <Link to="/notificationsmtps/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/notificationsmtps/edit/${encodeURIComponent(item["@id"])}`}>
