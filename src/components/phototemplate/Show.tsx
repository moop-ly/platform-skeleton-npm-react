...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/phototemplates/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show PhotoTemplate {item && item["@id"]}</h1>
      ...
      <Link to="/phototemplates/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/phototemplates/edit/${encodeURIComponent(item["@id"])}`}>
