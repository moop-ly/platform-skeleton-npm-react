...
const ShowView = ({
  ...
  return (
    <div>
      <h1>Show Webhook {item && item["@id"]}</h1>
        ...
        <table className="table table-responsive table-striped table-hover">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{item["event"]}retrieved</td>
            </tr>
            <tr>
              <th scope="row">endpoint</th>
              <td>{item["endpoint"]}retrieved</td>
            </tr>
          </tbody>
        </table>
      )}
      <Link to="/webhooks/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/webhooks/edit/${encodeURIComponent(item["@id"])}`}>
