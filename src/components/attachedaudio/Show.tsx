...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/attachedaudios/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show AttachedAudio {item && item["@id"]}</h1>
      ...
      <Link to="/attachedaudios/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/attachedaudios/edit/${encodeURIComponent(item["@id"])}`}>
