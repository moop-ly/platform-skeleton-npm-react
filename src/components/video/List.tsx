...
const ListView = ({ error, loading, retrieved }: ListProps) => {
  ...
  items.map((item) => {
    if ('contentUrls' in item) {
      delete item['contentUrls'];
    }
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item["@id"]}>
              ...
              <td>
                <Links
                  items={item["attachments"].map((ref: any) => ({
                    href: `/attachment-videos/show/${encodeURIComponent(ref)}`,
                    name: ref,
                  }))}
                />
              </td>
              ...
              <td>
                <Link to={`/videos/show/${encodeURIComponent(item["@id"])}`}>
                  <span className="fa fa-search" aria-hidden="true" />
                  <span className="sr-only">Show</span>
                </Link>
              </td>
              <td>
                <Link to={`/videos/edit/${encodeURIComponent(item["@id"])}`}>
...
const List = () => {
  ...
  const id = (page && decodeURIComponent(page)) || "videos";
