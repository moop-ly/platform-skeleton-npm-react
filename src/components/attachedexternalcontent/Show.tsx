...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/attachedexternalcontents/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show AttachedExternalContent {item && item["@id"]}</h1>
      ...
      <Link to="/attachedexternalcontents/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link
          to={`/attachedexternalcontents/edit/${encodeURIComponent(
            item["@id"]
          )}`}
        >
