...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/documenttemplates/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show DocumentTemplate {item && item["@id"]}</h1>
      ...
      <Link to="/documenttemplates/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/documenttemplates/edit/${encodeURIComponent(item["@id"])}`}>
