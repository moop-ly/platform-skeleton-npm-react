...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/attachedphotos/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show AttachedPhoto {item && item["@id"]}</h1>
      ...
      <Link to="/attachedphotos/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/attachedphotos/edit/${encodeURIComponent(item["@id"])}`}>
