...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/attachmentaudios/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show AttachmentAudio {item && item["@id"]}</h1>
      ...
      <Link to="/attachmentaudios/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/attachmentaudios/edit/${encodeURIComponent(item["@id"])}`}>
