...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/videofiles/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show VideoFile {item && item["@id"]}</h1>
      ...
      <Link to="/videofiles/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/videofiles/edit/${encodeURIComponent(item["@id"])}`}>
