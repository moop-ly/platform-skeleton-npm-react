...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/documentfiles/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show DocumentFile {item && item["@id"]}</h1>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <Links
                  items={{
                    href: `/document-templates/show/${encodeURIComponent(
                      item["template"]
                    )}`,
                    name: item["template"],
                  }}
      ...
      <Link to="/documentfiles/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/documentfiles/edit/${encodeURIComponent(item["@id"])}`}>
