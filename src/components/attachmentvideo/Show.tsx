...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/attachmentvideos/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show AttachmentVideo {item && item["@id"]}</h1>
      ...
      <Link to="/attachmentvideos/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/attachmentvideos/edit/${encodeURIComponent(item["@id"])}`}>
