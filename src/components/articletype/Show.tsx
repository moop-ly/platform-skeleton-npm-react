...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/articletypes/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show ArticleType {item && item["@id"]}</h1>
      ...
      <Link to="/articletypes/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/articletypes/edit/${encodeURIComponent(item["@id"])}`}>
