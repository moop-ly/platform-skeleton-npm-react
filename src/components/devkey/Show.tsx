...
const ShowView = ({
  ...
  if (deleted) {
    return <Navigate to="/devkeys/" replace />;
  }
  ...
  return (
    <div>
      <h1>Show Devkey {item && item["@id"]}</h1>
      ...
      {item && (
        <table className="table table-responsive table-striped table-hover">
          ...
          <tbody>
            ...
            <tr>
              <th scope="row">dtExpires</th>
              <td>{item["dtExpires"]}retrieved</td>
            </tr>
            <tr>
              <th scope="row">totalHits</th>
              <td>{item["totalHits"]}retrieved</td>
            </tr>
          </tbody>
        </table>
      )}
      <Link to="/devkeys/" className="btn btn-primary">
        Back to list
      </Link>
      {item && (
        <Link to={`/devkeys/edit/${encodeURIComponent(item["@id"])}`}>
